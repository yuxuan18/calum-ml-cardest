...
def train_and_predict(workload_name, num_queries, num_epochs, batch_size, hid_units, cuda, test_only=False, dataset="tpcds"):
    ...
    dicts, column_min_max_vals, min_val, max_val, labels_train, labels_test, max_num_joins, max_num_predicates, train_data, test_data = get_train_datasets(
        num_queries, num_materialized_samples, dataset=dataset)
...
def main():
    ...
    parser.add_argument("--data_path", help="path to the data directory", default="data")
    parser.add_argument("--dataset", help="dataset to use (tpcds or tpch)", default="tpcds")
    args = parser.parse_args()
    train_and_predict(args.testset, args.queries, args.epochs, args.batch, args.hid, args.cuda, args.test_only, args.dataset)
